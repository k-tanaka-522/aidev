...
**基本方針**: ライフサイクル別 + 層別のハイブリッド構成

```
infra/cloudformation/
├── persistent/          # 削除しないリソース
│   ├── network.yaml    # VPC, Subnet (推定250行)
│   └── database.yaml   # RDS (推定350行)
└── ephemeral/          # 削除可能なリソース
    ├── compute.yaml    # 親スタック (推定200行)
    └── nested/
        ├── ecs.yaml    # ECS Cluster, Service (推定250行)
        └── alb.yaml    # ALB, TargetGroup (推定200行)
```

**分割理由**:
- `compute.yaml` は単体で800行を超えるため、ネスト構成に分割
- ライフサイクルが異なるリソース（VPC vs ECS）は分離
- 各ファイルは300行以内を目標（推定値を記載）
