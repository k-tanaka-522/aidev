...
**IMPORTANT: アプリケーション設計とインフラ設計でディレクトリを分けてください**
...
5. **役割分担の明確化**:
   - `app/` → Coder が参照（アプリケーション実装）
   - `infra/` → SRE が参照（インフラ構築）
...
- **アプリケーション設計とインフラ設計を必ず分離してください**
  - `docs/03_基本設計/app/` → アプリケーション設計（Coder向け）
  - `docs/03_基本設計/infra/` → インフラ設計（SRE向け）
...
使用する技術「〇〇」の技術標準ファイルが存在しません。
既存の技術標準（例: Python, TypeScript等）とその技術の特徴を踏まえて、
新規に技術標準を作成してもよろしいでしょうか？

作成する場合の配置先:
- アプリ言語: `.claude/docs/40_standards/41_app/languages/[技術名].md`
- フレームワーク: `.claude/docs/40_standards/41_app/frameworks/[技術名].md`
- IaC: `.claude/docs/40_standards/42_infra/iac/[技術名].md`
```

2. **PM承認後、技術標準を作成**
   - 既存の類似技術標準（最も近い言語/フレームワーク）を Read
...
技術標準「[技術名]」を作成しました。
配置場所: `.claude/docs/40_standards/[パス]/[技術名].md`
...
- [ ] 使用する技術の標準ドキュメント（`.claude/docs/40_standards/`）を参照したか？
- [ ] 設計書の内容が技術標準に準拠しているか？
