...
## 🔒 サブエージェントの技術的制限（2025年仕様）

### 1. ネスト禁止

**制限**: サブエージェントがさらにTaskツールでサブエージェントを呼び出すことは**不可**

**理由**: Claude Codeアーキテクチャの制約

**影響**:
- サブエージェントは「実行者」であり「オーケストレーター」ではない
- 階層的なタスク分解はPMが行う必要がある

**解決策**:
```
❌ NG: PM → Architect → Coder（ネスト）

✅ OK: PM → Architect
        PM → Coder（PMが両方をオーケストレート）
```

**ベストプラクティス**:
- PMが全サブエージェントを直接管理
- 複雑なタスクは事前にPMがサブタスクに分解
- サブエージェント間の依存関係はPMが調整

---

### 2. 並列実行上限

**制限**: 最大**10タスク**まで同時実行可能。超過分はキューイング

**影響**:
- 10個を超えるサブエージェントを同時に起動すると、11個目以降は待機
- 大規模な並列処理は段階的に実行

**ベストプラクティス**:
- **重要タスクを優先的に並列化**
  - 例: 設計フェーズで Architect, SRE, Security を並列実行
- 軽量タスクは順次実行でも可
  - 例: ドキュメント生成は逐次でも問題なし
- 10タスク以上必要な場合は、グループ化して段階実行
  - 例: フェーズ1（5タスク）→ フェーズ2（5タスク）
...
**制限**: 各サブエージェントは約**200Kトークン**の独自コンテキストを持つ

**影響**:
- サブエージェント間でコンテキストは共有されない
- PMのコンテキストもサブエージェントには自動的に渡らない

**解決策**:
- **必要な情報はTask promptで明示的に渡す**
  ```markdown
  Task prompt例:
  ...
  - 設計方針: [PMがユーザーから確認した内容]

  ...
  ```

**ベストプラクティス**:
- サブエージェントに必要なファイルパスを明示
- 前提条件・制約を明確に記載
- 過去の決定事項を要約して渡す
- 「前回の会話」は参照できないと認識
...
Claude Code には以下の**組み込みサブエージェント**が用意されています：

| 名前 | 用途 | 特徴 | モデル |
|------|------|------|--------|
| **general-purpose** | 複雑なマルチステップタスク | 汎用、探索と実行両対応 | 指定可 |
| **Explore** | コードベース検索・分析 | **読み取り専用**、探索特化 | Haiku（高速） |
| **Plan** | プランモード時のリサーチ | プランモード専用 | 指定可 |

**使い分け**:
- **Explore**: 「〇〇の実装はどこ？」等の探索タスク
  - メリット: 高速（Haiku）、コスト効率良
  - 制限: 読み取り専用（編集不可）

- **general-purpose**: 複雑な調査・実装タスク
  - メリット: 柔軟性が高い
  - デメリット: カスタムエージェントほど専門性なし

- **Plan**: プランモード時のみ自動使用
  - PMが明示的に使う必要はなし

**aidev での方針**:
- カスタムエージェント（Consultant, Architect等）を優先
- 探索のみが必要なら Explore 使用検討
- general-purpose は例外的に使用

---

### 5. 1レスポンス完結の原則

**制限**: サブエージェントは**1つのレスポンス**を返したら終了
...
- 複数ファイルの処理は**一括指示**
  ```
  ✅ 正しい委譲:
  「以下の9ファイルをすべてクリーンアップしてください（1回で全完了）」

  ❌ 間違った委譲:
  「まず1ファイルをクリーンアップして、完了したら次を教えてください」
  ```

- 大規模タスクは事前に分割してからPMが並列委譲
...
**レビュー状態**: Draft
**最終更新**: 2025-11-26（技術的制限を追加）
**次のアクション**: 各サブエージェント AGENT.md の制限事項反映
