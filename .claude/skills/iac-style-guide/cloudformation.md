...
> **v1前提記述の書き換え（M2、02文書版1.7 14.2節の完了条件）**: 「📋 設計書とテンプレートの
> 区別」節がv1の「設計フェーズ」「実装フェーズ」という直列工程名と`docs/03_基本設計/`という
> v1のディレクトリ構造を前提にしていたため、v2のZone3成果物`docs/04_インフラ設計/`
> （03_成果物体系定義書3.6節の04-01〜04-11）を前提にした記述へ書き換えた。ファイル分割の
> 考え方自体（複数ファイルに分割する理由、3原則）は技術的内容でありv1/v2で変わらないため
> 変更していない。
...
| 成果物 | 生成ゾーン（v2） | ファイル構成 | 目的 |
|-------|--------|-----------|------|
| **インフラ設計書（`docs/04_インフラ設計/`）** | Zone3（`reverse-doc`によるas-built生成。02文書4.2.1節） | 常に複数ファイルに分割（03文書3.6節の04-01〜04-11） | レビューしやすさ、並行作業 |
| **CloudFormation テンプレート（`infra/{layer}/`）** | Zone2（`execute`による硬化） | ファイル分割3原則に基づく | メンテナンス性、変更リスク最小化 |

v1は「設計フェーズで基本設計書を書いてから実装フェーズでテンプレートを作る」という直列
順序だったが、v2はZone1〜2でCloudFormationテンプレート（`infra/{layer}/`）を先に硬化させ、
`docs/04_インフラ設計/`はZone3到達まで生成しない（02文書4.2.1節「Zone3まで空である状態が
正常」）。したがって以下の「基本設計書のファイル分割」は、**Zone3の`reverse-doc`が
`docs/04_インフラ設計/`を生成する際に踏襲すべきファイル分割の目安**として読み替える。

### インフラ設計書のファイル分割（Zone3生成時の目安、03文書3.6節の項番に整合）

`docs/04_インフラ設計/`は、規模に関わらず以下の構成で複数ファイルに分割する（03文書3.6節が
定める04-01〜04-11の項番体系に対応させた例）：

```
docs/04_インフラ設計/
├── 04-01_システム構成図.md            # 全体構成図、システム構成要素
├── 04-02_ネットワーク設計.md          # VPC、サブネット、ルーティング
├── 04-03_セキュリティ設計.md          # Security Groups、WAF、認証、暗号化（インフラ層）
├── 04-04_可用性設計.md                # 条件付き必須（可用性クラス規定案件）
├── 04-05_監視設計.md                  # CloudWatch、SNS、通知
├── 04-06_バックアップ設計.md          # 条件付き必須（永続データを持つ案件）
├── 04-07_DR設計.md                    # 条件付き必須（可用性クラス上位のみ）
├── 04-08_コスト見積.md                # Zone0決定ログからの転記
├── 04-09_環境設計.md                  # dev/stg/prd等の環境差分
├── 04-10_IaC方針.md                   # ⭐ ファイル分割3原則、ディレクトリ構造
├── 04-11_CI_CD設計.md                 # 条件付き必須（自動デプロイ基盤構築案件）
└── 02_詳細設計/04-D-{リソースID}.md    # GZ2到達時オンデマンド生成（03文書5章）
```

**重要**: `04-10_IaC方針.md`には、`infra/{layer}/`実装（Zone2）で使用する**ファイル分割3原則**と**ディレクトリ構造**を必ず記載する。以降の「ファイル分割の3原則」節はCloudFormationテンプレート自体（`infra/{layer}/`、Zone2）に適用する規約であり、v1/v2で変わらない技術的内容のため以下は変更していない。
