> 移行元: `.claude/docs/40_standards/42_infra/cicd/github_actions.md`（02文書4.4節、新設ディレクトリ `iac-style-guide/cicd/`）
...
**参考**: 検証完了レポート - OIDC認証セットアップ（旧検証プロジェクトの記録。本リポジトリには存在しないため参照リンクは無効。移管時に判明した外部参照切れ）
...
**参考**: 検証完了レポート - パラメーターフィルタリング（同上、外部参照切れ）
...
**参考**: CloudFormation標準（本Skill内 `../cloudformation.md`）
...
- 検証完了レポート（Sample-AWS-SubAgent、外部参照切れ）
- CICD設計（Sample-AWS-SubAgent、外部参照切れ）
