> 移行元: `.claude/docs/40_standards/41_app/api_design.md`（02文書4.4節、新設 `contract-design/API_DESIGN_STANDARD.md`。M1で移管漏れだった項目をM2で実施）
> RESTful原則・バージョニング・OpenAPI提供方針は「契約モックをどう設計するか」というZone1の
> 規約であり、実装のコードスタイルではないため`code-style-guide`ではなく`contract-design`の
> 自動参照ファイルとする（02文書4.4節の判断理由）。
> **v1前提記述の書き換え**: 本ファイルの技術的内容（RESTful原則、ステータスコード等）は
> v1/v2で変わらないため無変更で移管した。末尾の「参照」節のみ、移行元がv1の
> `.claude/docs/10_facilitation/2.3_設計フェーズ/2.3.4_API設計/`への相対参照だったため、
> v2の`contract-design`（Zone1、レーンB）を前提にした文言へ書き換えた。
...
**参照**: 本標準は`contract-design`（Zone1、レーンB。`docs/00_プロジェクト管理・ガバナンス/decisions/contracts/.claude/skills/contract-design/SKILL.md`）から自動参照される。データ形状（型・制約）の最終決定は`code-style-guide/data/DATABASE_STANDARD.md`（Zone2、`src/backend`主）と整合を取ること（02文書4.4節）。
