...
│   ├── security-guard/{SKILL.md, SECURITY_STANDARD.md}   # SECURITY_STANDARD.mdは版1.6で追加、`paths`によるpassive自動参照を併設（4.4節）
│   ├── doc-style-guide/SKILL.md
│   ├── code-style-guide/{SKILL.md, languages/*.md, frameworks/*.md, data/*.md}   # frameworks/・data/は版1.6で追加（4.4節）
│   ├── iac-style-guide/{SKILL.md, *.md, cicd/*.md, testing/*.md}   # cicd/・testing/は版1.6で追加（4.4節）
...
| `41_app/frameworks/{react_nextjs,flutter}.md` | `code-style-guide/frameworks/*.md`（新設） | `src/frontend/execute`（react_nextjs）、`src/mobile/execute`（flutter）（Zone2、`paths`をディレクトリ別に設定） | フレームワーク規約は言語規約と同様「実装時にどう書くか」であり、`languages/`と同列の自動参照ファイルとして`code-style-guide`に集約する。レーンBの`execute`側に個別ロジックを持たせると規約の所在が分散するため採らない |
...
| `42_infra/cicd/{cicd-security,github_actions}.md` | `iac-style-guide/cicd/*.md`（新設） | `infra/**`・`.github/workflows/**`を`paths`に追加した`iac-style-guide`（sre主） | CI/CDパイプライン定義は「構築物の一種」でありIaCと同じ性質を持つ。SRE専用の新規Skillを立てると規約の所在が分散するため、既存の`iac-style-guide`に`cicd/`を追加する形で吸収する |
| `42_infra/testing/infra-test-structure.md` | `iac-style-guide/testing/INFRA_TEST_STANDARD.md`（新設） | `infra/**`・`tests/integration/**`を`paths`に追加した`iac-style-guide` | インフラテストの構造規約も「IaCに関する規約」の一部として`iac-style-guide`に一元管理する |
| `49_common/security.md` | `.claude/skills/security-guard/SECURITY_STANDARD.md`（新設） | `security-guard`が正本として保持し、`code-style-guide`/`iac-style-guide`/`contract-design`/`ui-style-guide`の各SKILL.mdからはリンク参照する（内容を転記・複製しない） | 全レーン横断のセキュリティ基準であるため、既存の横断Skill`security-guard`（5.1節）に統合する。`security-guard`は`context: fork`による能動的検査と、`paths`によるpassive自動参照の二役を担う構成とする（両者を1つのSkillに併設できるかは要検証、15章#23） |
...
| security-guard | `.claude/skills/security-guard/SKILL.md` | `context: fork`＋`paths`（版1.6で併設、4.4節） | コンプライアンス検査（能動）、`SECURITY_STANDARD.md`（`49_common/security.md`の移管先、版1.6新設）の自動参照（受動） | orchestrate、`src/**`・`infra/**`・`decisions/contracts/**`・`prototypes/**`（自動） | qa |
...
| code-style-guide | `.claude/skills/code-style-guide/` | `user-invocable: false`, `paths` | コーディング規約（`languages/`）、フレームワーク規約（`frameworks/`、版1.6新設）、DB設計規約（`data/`、版1.6新設。4.4節） | `src/**` の execute/review（自動） | — |
| iac-style-guide | `.claude/skills/iac-style-guide/` | `user-invocable: false`, `paths` | IaC規約（直下）、CI/CD規約（`cicd/`、版1.6新設）、インフラテスト規約（`testing/`、版1.6新設。4.4節） | `infra/**`・`.github/workflows/**`・`tests/integration/**` の execute/review（自動） | — |
...
| M2 | ゾーン/レーンSkill実装（`prototypes/`の`mockup-*`3種、`contract-design`（新設）、`src`/`infra`の`execute`/`review`、`sync-check`、`sync-ledger-guard.js`（新設、版1.5。`sync-check`が`00-05`へ追記する機構と対になるためM2で実装する、版1.6で明記）、6種ID体系の採番） | ミニゲートが機能単位で1件通しで動作確認済み。`00-02`〜`00-05`台帳への登録（`00-05`同期点記録台帳への追記漏れ検知を含む）が確認済み |
...
| 23 | - | `security-guard`への`context: fork`と`paths`の併設可否（版1.6で追加） | 4.4節・5.1節で`security-guard`に能動的検査（`context: fork`）と受動的自動参照（`paths`）の二役を持たせる設計を採ったが、1つのSkillのfrontmatterにこの2属性を両立できるかはClaude Codeの仕様として未確認である。両立できない場合、`SECURITY_STANDARD.md`の自動参照のみを担う別Skill（例: `security-style-guide`）へ分離する必要がある |
