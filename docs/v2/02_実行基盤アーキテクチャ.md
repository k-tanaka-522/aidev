...
graph TB
    ...
    subgraph L4["強制層"]
        ...
        HOOK[hooks PreToolUse/PostToolUse/Stop（ロール識別方式は7.4節、版1.9時点で未確定）]
    end
...
├── hooks/
│   ├── lint-guard.js
│   ├── role-boundary-guard.js         # 識別子取得方式は7.4節で未確定（版1.9、PM確認中）。7.1.2節参照
...
#### 7.1.2 ロール別パス制御（`role-boundary-guard.js`、識別子取得方式は7.4節で未確定）

`docs/02_**`〜`docs/04_**`, `src/**`, `infra/**`, `tests/**`, `decisions/**`, `prototypes/**` への Write/Edit は、settings.jsonのpermissionsではなく`role-boundary-guard.js`（PreToolUse hook）が呼び出し元の識別子（具体的な取得方式は7.4節、版1.9時点で未確定）を見て許可パスを判定する。既定は**全ロール拒否のホワイトリスト方式**とし、以下の表に無いパスへの書込、および識別子が判定不能な場合は`exit 2`で拒否する（MUST、fail closed。7.4節）。

| ロール（7.4節の識別子が示す値） | 許可される書込パス | 備考 |
|---|---|---|
| （未設定＝メインスレッド＝PM） | `docs/00_プロジェクト管理・ガバナンス/**`（`decisions/**`を除く）、`.claude-state/**` | 識別子が付与されないことでメインスレッドと判定する（要検証、15章#19） |
...
| 2 | `role-boundary-guard.js` | `PreToolUse`（Edit\|Write） | ロール境界違反。7.1.2節の許可パス表と、7.4節が定める識別子取得方式（版1.9時点で未確定。方式A/Bいずれか、PM確認中）を突合 | `exit 2`（識別子が判定不能な場合も`exit 2`、fail closed） | 版1.4は「`agent_id`/`agent_type`がhook共通入力に含まれる」とweb調査に基づき記載していたが、M3の実機観測でこれは誤りと判明した（版1.9、事実1）。7.4節参照 |
...
### 7.4 ロール境界の機構化 — 実機観測による前提の訂正（版1.9、方式は未確定）

**版1.9での訂正（重要）**: 版1.4は「web調査によりhook共通入力に`agent_id`/`agent_type`が含まれることを確認した」としていたが、**M3実装時にPMが実機で観測した結果、この2フィールドはhookの共通入力には存在しないことが判明した**。5章冒頭の`paths`書式問題と同型の失敗（web上の解説記事の記述と実装の実物が異なっていた）であり、以後、hookペイロード等Claude Codeの内部仕様に関わる事実はweb調査のみで確定させず、可能な限り実機観測で裏を取ることを徹底する（MUST、5章冒頭のMUST注記と同一の原則をここにも適用する）。

**実機で確認済みの事実（事実1）**: サブエージェントの識別子は、hookの共通入力フィールドではなく、**`Agent`ツール自体の完了時ペイロード**（`tool_response`、camelCase）に載る。

```
tool_response.agentId    = "ac0fdd5cd34b7e884"
tool_response.agentType  = "Explore"
tool_response.status     = "completed"
tool_response.content    = （リスト。完了報告の本文）
```

これは`PostToolUse`（`tool_name: "Agent"`）で観測される。**事実3**: `tool_name`は`Task`ではなく`Agent`である。本書内で`Task`をhookのmatcherとして記載していた箇所（旧7.3節#7・旧8.2.4節）は誤りであり、`Agent`に訂正した。

この事実は8.2.4節の`task-boundary-guard.js`（決定ログ機構・段3）にとっては朗報である。同hookは`Agent`ツールの完了を検知した時点で`tool_response.content`（完了報告テキスト）と`tool_response.agentId`/`agentType`（起票元の特定）の両方を1つのペイロードから取得でき、旧15章#23が要検証としていた「Task完了報告テキストがどのフィールドに載るか」という疑問はこれで解消した（15章の解消済み項目へ移した）。

**未確定（事実2、最優先・PM確認中。本節はここを確定させない）**: しかし`role-boundary-guard.js`が実際に判定したいのは「サブエージェントが`src/`等へ`Edit`/`Write`しようとした瞬間」であり、これは`Agent`ツールの完了イベントとは別の、**`Edit`/`Write`ツール自体のhookイベント**である。`agentId`/`agentType`に相当する識別子がこの`Edit`/`Write`のペイロードにも載るかどうかは、本版執筆時点でPMが実機確認中であり、**結果が出るまで本節を確定させない**。以下の2方式を並記し、確認結果に応じてどちらかを正本として採用する（PMが結果を追って連絡し、次版で確定させる）。

- **方式A（`Edit`/`Write`ペイロードに識別子が載る場合）**: `role-boundary-guard.js`は`Edit`/`Write`の`PreToolUse`ペイロードから直接その時点の識別子を読み、7.1.2節の許可パス表と突合する。追加の状態管理は不要
- **方式B（載らない場合。フォールバック）**: `active-agent.json`方式を採用する。`orchestrate`が`Agent`ツールを呼び出す直前に、これから呼び出すサブエージェントのロール名を`.claude-state/active-agent.json`（例: `{"active_role": "app-architect", "since": "..."}`）へ記録し、`Agent`ツール呼び出し完了後（`tool_response`受領後）にクリアする。`role-boundary-guard.js`は`Edit`/`Write`の`PreToolUse`時にこのファイルを読み、`active_role`を7.1.2節の許可パス表と突合する。**既知の限界**: (a) `orchestrate`がファイルへの記録を怠ると境界が機能しない更新漏れリスクを持つ、(b) 複数のサブエージェントが並行実行される場合（Workflowツールの`parallel`構文等）、単一の`active_role`では同時に複数のロールを表現できず、配列・スタック構造への拡張が別途必要になる

**方式の選定基準**: 方式Aが成立するなら優先して採用する（状態管理コストが無く、更新漏れの構造的リスクを持たないため）。方式Aが成立しない場合のみ方式Bを採用し、上記(b)の並行実行時の拡張を別途設計する。

**判定できない場合の既定動作（事実2の確定を待たず先に決める、MUST。指摘B-8）**: 方式A・方式Bのいずれを採用するにせよ、識別子が取得できない場合、または値が7.1.2節の表に無い未知の値である場合は**拒否する（fail closed）**。7.1.2節の許可パス表に無いパスへの書込を許可しないのと同じ「ホワイトリスト方式」の原則を、識別子取得の失敗時にも一貫して適用する（MUST）。判定不能を許可（fail open）にすると、hookの実装不備や識別子取得の一時的失敗によってロール境界の強制力そのものが無効化されてしまうためである。

**残る不確実性（要検証、15章#19）**: 識別子が未設定（またはメインスレッド相当の値）の場合にPMと一意に判定してよいかは、Claude Codeが将来`--agent`セッション種別を導入した場合に崩れる可能性がある。この判定ロジックの信頼性は運用実績を踏まえて再確認する。
...
| `.claude/hooks/prevent-pm-layer-violation.sh` | ロール境界hook | 変換 | `role-boundary-guard.js`（識別子取得方式は7.4節参照、版1.9時点で未確定。対象パスに`prototypes/`,`decisions/`追加） |
...
| M3 | Hooks/Permissions強制層の実装。**`role-boundary-guard`の識別子取得方式は7.4節の方式A/Bいずれかで確定する（版1.9時点で未確定、PM確認中。結果が出るまでM3は方式Bを暫定採用して進めてよい、MAY）**。**`gate-transition-guard.js`は10.2.1節が正本化した`current-zone.json`スキーマ（版1.8で確定）に従って実装する**。**`task-boundary-guard.js`（8.2.4節）は`Agent`ツール完了ペイロードの`tool_response.content`/`agentId`/`agentType`を参照して実装する（版1.9で実機確認済み、事実1）**。全hookの`settings.json`登録は`node`前置を徹底する（MUST、版1.9、事実5） | `settings.json`がプロジェクト共有としてコミットされ、7章の全hookが動作確認済み。`gate-transition-guard.js`が10.2.1節の遷移判定表どおりに動作することを確認済み（版1.8）。`task-boundary-guard.js`が実機確認済みのフィールド名を参照して動作していることを確認済み。**`role-boundary-guard.js`の識別子取得方式（方式A/B）が確定し動作確認済みであること**（版1.9で追加）。`settings.json`の変更がセッション再起動なしで反映されることを確認済み（版1.9、事実4） |
...
| 19 | - | `role-boundary-guard.js`のメインスレッド判定ロジックの信頼性 | 識別子が未設定＝PMという判定が、Claude Codeの将来リリースでも成立し続けるか（7.4節）。加えて、そもそもどの方式（方式A/B、事実2）を採用するか自体が版1.9時点で未確定であり、本項目は方式確定後に再評価する |
...
| 26 | - | **（最優先・未確定）`role-boundary-guard.js`の識別子取得方式（版1.9で追加、事実2）** | `Edit`/`Write`のhookペイロードにサブエージェント識別子（`agentId`/`agentType`相当）が載るかどうかを本版執筆時点でPMが実機確認中である。載れば方式A（ペイロード直読）、載らなければ方式B（`active-agent.json`方式）を採用する（7.4節に両論併記済み）。**M3の実装（7章の全hook動作確認）はこの結果が出るまで完了と判定できない。** PMからの確認結果を受けて次版で確定させること |
