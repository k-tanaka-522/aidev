...
| M3 | Hooks/Permissions強制層の実装。`role-boundary-guard`を`agent_id`/`agent_type`参照方式で稼働（版1.4）。**`gate-transition-guard.js`は10.2.1節が正本化した`current-zone.json`スキーマ（版1.8で確定）に従って実装する**。**`task-boundary-guard.js`（8.2.4節）が実機のTask完了ペイロードのどのフィールド（`tool_response`等）に完了報告テキストが載るかを確認する（版1.7で追加、指摘2。15章参照）** | `settings.json`がプロジェクト共有としてコミットされ、7章の全hookが動作確認済み。`gate-transition-guard.js`が10.2.1節の遷移判定表どおりに動作することを確認済み（版1.8）。`task-boundary-guard.js`が実機確認済みのフィールド名を参照して動作していることを確認済み |
...
| 23 | - | Task完了報告テキストの格納フィールド名（`task-boundary-guard.js`、版1.7で追加） | 7.4節で確認済みなのは`agent_id`/`agent_type`という呼び出し元識別フィールドであり、Task完了時の返り値テキスト自体がPostToolUseペイロードのどのフィールド（`tool_response`等）に載るかは未確認である。実装は複数候補を順に試す防御的な作りにしているが、フィールド名を誤ると常に誤検知し続けるリスクが残る。決定ログ機構の第3段（8.2.4節）の核心に関わるため、M3の完了条件に実機確認を追加した（14.2節） |
| 24 | - | `src_unlocked`のプロジェクト単位近似の限界（版1.8で追加） | 10.2.1節が採用した`src_unlocked`は「プロジェクト全体で最初の契約確定後は`src/**`を許可する」という近似であり、個々の`HB-ID`/機能単位でZone1・Zone2を完全に締め出す機構ではない。ある機能がまだZone1段階でも、他の機能が既にZone2へ進んでいれば当該app-architectは`src/**`へ書込可能になる。運用実績を踏まえ、機能単位の厳密な制御が実際に必要かどうかを再検討する必要がある |
| 25 | - | `sync-check --source=impl`の抽出パターンの網羅性（版1.8で追加） | 10.3節で確定した正規表現ベースのORM抽出（`Column(...)`、`@Column()`、`models.CharField`等）は代表的な記法のみを対象とし、対応外のORM・素朴なSQLベースの実装では抽出漏れが生じる。9.1.1節と同様「フレームワーク別アダプタを拡張していく」設計とするが、初期実装でどこまでのフレームワークをカバーすべきかは要検証である |
| 26 | 03 | `00-03`/`00-04`/`00-05`の列定義正本化、および`00-02`「経路追加ログ」の正式な列反映方式 | M2実装により、`00-03`（バッチトレーサビリティ台帳）・`00-04`（運用項目一覧）・`00-05`（同期点記録台帳）の列は、01/03/04文書の箇条書き記述をもとに実装者が独自に設計する状態になっている（`00-02`は03文書3.2.1節に列定義があるのに対し非対称）。複数の実装者が触れると列定義が食い違うリスクが高いため、03文書側で正本の列定義を与えられたい。あわせて、10.1.4節（版1.8）で新設した`00-02`内「経路追加ログ」（追記専用でBAT-ID等の後発経路情報を記録する仕組み）についても、03文書のカタログ・列定義に正式に組み込まれたい |
